...
Functions:
  ...
  - lopo_window_threshold_report(X, y, meta, participant_col, plot,
    ...
    confidence intervals, optionally plotting a per-class distribution
    summary and violin plot of the ratio feature, threshold stability,
    and a pooled confusion matrix.
"""
...
def lopo_window_threshold_report(
    ...
    for window in windows:
        ...
        if plot and "Class" in wdf.columns and "Personalised SigMMD Ratio" in wdf.columns:
            group_col = "Class"
            value_col = "Personalised SigMMD Ratio"
            order = ["Stable", "Progressive"]

            metrics = (
                wdf.groupby(group_col)[value_col]
                .agg(
                    n="count",
                    mean="mean",
                    median="median",
            ...
            print(f"\n[window {window}] Personalised SigMMD Ratio distribution by class:")
            print(metrics)
            ...
            sns.violinplot(
                ...
                order=order,
                inner="box",
                inner_kws=dict(box_width=5, whis_width=2, color="#111111"),
                linewidth=0.8,
                ax=ax,
            )
            ax.set_xlabel(group_col, fontsize=8.5, labelpad=3)
            ax.set_ylabel(value_col, fontsize=8.5, labelpad=3)
            ax.tick_params(axis="both", which="major", labelsize=7.5, length=3, width=0.8)
            ax.set_axisbelow(True)
            ax.margins(x=0, y=0)
            ax.set_title(f"Window {window}: ratio distribution by class", fontsize=8.5)
            plt.show()
